...
void draw(){
  ...
  for(int i = 0; i < bands; i++){
    ...
    //stroke(#fec763); 
    //line(i, height/2, i, (height/2) - spectrum[i]*(height/2)*5);
    
    //lumen = (spectrum[i] * 1000);
    
    //strokeWeight(spectrum[i]*50);
    //point(height/2, height/2 - spectrum[i]*height*5);
    //point(height/2, height/2 + spectrum[i]*height*5);
    //point(height/2 + spectrum[i]*height*5, height/2);
    //point(height/2 - spectrum[i]*height*5, height/2);
    
    fill(0);
    strokeWeight(1);
    
    if ((spectrum[i]*height*3) > 750){
      stroke(#FDCB9E); 
    }else if((spectrum[i]*height*3) > 450){
      stroke(#00B7C2); 
    ...
    //ellipse(width/2, height/2, spectrum[i]*height*3, spectrum[i]*height*3);
    strokeWeight(spectrum[i]*100);
    point(height/2, height/4 - spectrum[i]*height*7);
    point(height/2, height/4 + spectrum[i]*height*15);
    point(height/2 + spectrum[i]*height*7, height/4);
    point(height/2 - spectrum[i]*height*7, height/4);
